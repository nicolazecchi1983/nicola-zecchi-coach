alter table public.match_gps_player_metrics
  add column if not exists metric_values jsonb
  not null default '{}'::jsonb
  check (jsonb_typeof(metric_values) = 'object');

comment on column public.match_gps_player_metrics.metric_values is
  'Canonical extensible GPS metric payload. Keys are STAFF metric identifiers; values are numeric metric observations. Legacy scalar columns remain for backward compatibility.';

update public.match_gps_player_metrics
set metric_values = jsonb_strip_nulls(
  jsonb_build_object(
    'restingHeartRate', resting_heart_rate,
    'maxHeartRate', max_heart_rate,
    'maxSpeedMs', max_speed_ms,
    'distanceMaxSpeedKm', distance_max_speed_km,
    'averageSpeed', average_speed,
    'accelerationMs2', acceleration_ms2,
    'accelerationCount', acceleration_count,
    'decelerationCount', deceleration_count,
...
begin
  ...
  select
    ...
    jsonb_strip_nulls(
      jsonb_build_object(
        'restingHeartRate', row_data.resting_heart_rate,
        'maxHeartRate', row_data.max_heart_rate,
        'maxSpeedMs', row_data.max_speed_ms,
        'distanceMaxSpeedKm', row_data.distance_max_speed_km,
        'averageSpeed', row_data.average_speed,
        'accelerationMs2', row_data.acceleration_ms2,
        'accelerationCount', row_data.acceleration_count,
        'decelerationCount', row_data.deceleration_count,
        'distanceKm', row_data.distance_km
      )
    ) || coalesce(row_data.metric_values, '{}'::jsonb),

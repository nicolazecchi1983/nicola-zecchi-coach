...
const checks = [
  ...
  ['formation PDF preserves canonical starter order', runtime.includes('Array.from({ length: MATCH_LINEUP_STARTER_COUNT }') && lineup.includes('MATCH_LINEUP_STARTER_COUNT = 11') && /starters\.map/.test(runtime)],
  ['formation PDF preserves editable bench capacity and never invents 12-20 numbers', runtime.includes('Array.from({ length: MATCH_LINEUP_MAX_BENCH }') && lineup.includes('MATCH_LINEUP_MAX_BENCH = MATCH_CALLUPS_MAX_PLAYERS - MATCH_LINEUP_STARTER_COUNT') && runtime.includes("number: form.elements[`bench_number_${index}`]?.value || ''") && !runtime.includes('number: form.elements[`bench_number_${index}`]?.value || String(index + 12)') && /bench\.map/.test(runtime)],

...
const checks=[
 ...
 ['zero selections saveable when match exists but over-limit is blocked',events.includes('saveButton.disabled = !activeMatch?.id || overLimit')&&events.includes('MATCH_CALLUPS_MAX_PLAYERS')],
 ['PDF requires 1-20 selected players',events.includes('pdfButton.disabled = selected.length === 0 || overLimit')],
 ...
 ['saved callups are the only Formation roster source, including empty/unsaved states',model.includes('if (!callups?.persisted) return []')&&model.includes('return rosterPlayers.filter')],

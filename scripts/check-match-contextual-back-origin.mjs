...
const canonicalSections=['opponent-study','callups','our-team','opponent','analysis','match-center','match-statistics','match-report-workspace','post-match']
const checks=[
 ...
 ['origin writers remain entry-point only',(dashboard.match(/setItem\('staff-match-entry-origin'/g)||[]).length===1 && (library.match(/setItem\('staff-match-entry-origin'/g)||[]).length===1 && !analysis.includes("setItem('staff-match-entry-origin'") && !workspaceEvents.includes("setItem('staff-match-entry-origin'")],
 ['shared Match owner reads origin exactly once',(workspaceEvents.match(/getItem\('staff-match-entry-origin'/g)||[]).length===1 && workspaceEvents.includes("storage?.getItem('staff-match-entry-origin') === 'dashboard'")],
 ['dashboard destination',workspaceEvents.includes("['dashboard', 'Dashboard']")],
 ['library destination',workspaceEvents.includes("['match-library', 'Match Library']")],
 ['shared label hook',ui.includes('data-match-context-back-label') && workspaceEvents.includes("backButton.querySelector('[data-match-context-back-label]')")],
 ['contextual label',workspaceEvents.includes('`Torna alla ${destination[1]}`')],
 ['contextual navigation',workspaceEvents.includes('setActiveNavigation(destination[0])') && workspaceEvents.includes("storage?.setItem('nz-active-section', destination[0])") && workspaceEvents.includes('await setView(destination[0], destination[1])')],
 ['Analysis no longer owns shared Match return',!analysis.includes('staff-match-entry-origin')&&!analysis.includes('data-return-to-match-workspace')],
 ...
 ['internal Match navigation never clears origin',!analysis.includes("removeItem('staff-match-entry-origin')") && !workspaceEvents.includes("removeItem('staff-match-entry-origin')")],
 ['all operational Match sections remain canonical',canonicalSections.every((key)=>session.includes(`'${key}'`))],
 ['fallback remains Match Library',workspaceEvents.includes("? 'dashboard' : 'match-library'")],
 ['design system does not read storage',!ui.includes('staff-match-entry-origin')],
 ['no database persistence',!analysis.includes("from('staff-match-entry-origin')") && !workspaceEvents.includes("from('staff-match-entry-origin')") && !dashboard.includes("from('staff-match-entry-origin')") && !library.includes("from('staff-match-entry-origin')")],
 ['direct-entry gate validates shared contextual back',directEntryGate.includes('section back action is contextual') && directEntryGate.includes('back control exposes contextual copy hook') && directEntryGate.includes("workspaceEvents.includes(\"staff-match-entry-origin\")")],
 ...
 ['suite grows exactly to 259',Array.isArray(pkg.staffCheckSuite) && pkg.staffCheckSuite.length===259],
]
let failed=0

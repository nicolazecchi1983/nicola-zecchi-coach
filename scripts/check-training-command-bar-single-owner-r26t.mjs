...
add('command owner owns compact mobile action grid',mobile.includes('grid-template-columns: auto'));
add('command owner preserves single 44px action row',mobile.includes('grid-template-rows: 44px;')&&!mobile.includes('grid-template-rows: 44px 24px'));
add('command owner retires obsolete status-row gap',mobile.includes('row-gap: 0;')&&!mobile.includes('row-gap: 4px'));
add('obsolete 390px selector patch is retired',!mobile.includes('@media (max-width: 390px)'));
add('retired selector and Open action are absent',!owner.includes('.ts-open-sheet')&&!owner.includes('.ts-open-button'));
add('command wrapper is compact and content-driven',owner.includes('width: max-content')&&owner.includes('max-width: 100%'));
add('command owner preserves More 44px geometry',mobile.includes('.ts-more-menu,')&&mobile.includes('.ts-more-button'));
add('command owner no longer owns draft state',!owner.includes('.ts-draft-state--compact')&&!mobile.includes('grid-template-rows: 44px 24px'));

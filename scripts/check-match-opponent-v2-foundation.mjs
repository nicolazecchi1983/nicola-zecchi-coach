...
const checks = [
  ...
  ['loaded PDF hides inactive image preview without layout reservation', css.includes('.opponent-sheet-upload img[hidden] { display: none; }') && events.includes('opponentSheetPreview.hidden = true') && events.includes('opponentSheetDocument.hidden = false')],

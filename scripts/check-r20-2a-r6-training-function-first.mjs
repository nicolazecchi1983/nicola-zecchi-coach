...
const checks = [
  ...
  ['Rosa usa un solo status selector per giocatore', trainingPage.includes('data-player-status') && trainingPage.includes('data-player-row') && !trainingPage.includes('data-player-select="')],
  ['Toolbar TS è content-driven dopo il ritiro del browser embedded', commandBar.includes('width: max-content') && commandBar.includes('max-width: 100%') && !commandBar.includes('width: min(100%, 500px)')],
  ['Toolbar TS ha ritirato la larghezza legacy di Apri TS', !commandBar.includes('.ts-open-button') && !commandBar.includes('width: 84px')],
  ['Controlli toolbar condividono 44px', commandBar.includes('height: 44px') && commandBar.includes('min-height: 44px')],

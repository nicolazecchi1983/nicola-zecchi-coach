...
const checks = [
  ...
  ['edit lookup uses UUID-aware identity', app.includes('rosterPlayerIdentity(item) === button.dataset.rosterEdit')],
  ['profile lookup uses UUID-aware identity', app.includes('rosterPlayerIdentity(item) === button.dataset.playerProfile')],
  ['legacy profile read remains compatible', app.includes('appState.playerProfiles[identity] || appState.playerProfiles[legacyKey]')],

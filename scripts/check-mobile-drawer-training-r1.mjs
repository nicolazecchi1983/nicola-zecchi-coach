...
const checks = [
  ...
  ['Parallel work uses STAFF dark surface', layer.includes('.ts-manual-editor .ts-parallel-work') && layer.includes('background: var(--staff-color-bg-panel-raised)')],
  ['Parallel subgroups use dark controls', layer.includes('background: var(--staff-color-bg-control)')],
  ['Parallel groups stack on mobile', layer.includes('.ts-parallel-work-grid') && layer.includes('grid-template-columns: 1fr')],

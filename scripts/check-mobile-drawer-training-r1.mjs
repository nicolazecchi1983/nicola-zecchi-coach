...
const checks = [
  ...
  ['Training compact command cluster has a dedicated owner', commandBar.includes('grid-template-columns: auto') && commandBar.includes('.ts-command-actions') && commandBar.includes('.ts-more-menu')],
  ['Retired Open TS controls are absent from the command-bar owner', !commandBar.includes('.ts-open-button') && !commandBar.includes('.ts-open-sheet')],
  ['Draft state is retired from the command status row', !commandBar.includes('.ts-draft-state--compact') && !commandBar.includes('grid-template-rows: 44px 24px')],

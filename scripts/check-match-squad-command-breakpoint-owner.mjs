...
const checks=[
 ...
 ['1180 has no lineup leadership override',count(b1180,/\.match-squad-step \.lineup-list--selection \.lineup-leadership\s*\{/g)===0],
 ...
 ['lineup leadership stays base-owned through 1180',!b1180.includes('.lineup-leadership') && css.includes('.lineup-list--selection .lineup-leadership {')],
 ['dead 280px owner is gone',!b1180.includes('minmax(280px, 1fr)')],
 ['760 primary command has two scoped owners',count(b760,/\.match-squad-step \.squad-command-primary\s*\{/g)===2],
 ['760 primary command has exactly one one-column geometry owner',count(b760,/\.match-squad-step \.squad-command-primary\s*\{[\s\S]*?grid-template-columns:\s*1fr;/g)===1 && !b760.includes('.lineup-leadership')],

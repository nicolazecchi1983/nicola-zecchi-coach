...
const checks = [
  ...
  ['update guard ignores only the event being edited', !service.includes('pendingDeletionEventIds') && service.includes('ignoredIds')],
  ...
  ['training publish cannot bypass slot collision guard', !trainingService.includes('pendingDeletionEventIds') && /updateEvent\(existingEvent\.id, payload\)/.test(trainingService)],

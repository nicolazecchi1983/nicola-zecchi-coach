...
const benchStart=view.indexOf('<div class="bench-block bench-block--editable bench-block--full-width"')
...
const checks=[
 ...
 ['bench has explicit full-width editable hook',view.includes('bench-block--editable')&&view.includes('bench-block--full-width')&&css.includes('.bench-block--full-width')],
 ...
 ['formation and Distinta runtime hooks remain explicit',view.includes('name="formation"')&&view.includes('data-reset-formation')&&view.includes('data-leadership-select="captain"')&&view.includes('data-bench-select')&&view.includes('bench_number_${index}')],
 ['view owns no persistence or data-access shortcut',!view.includes('supabase')&&!view.includes('repository')&&!view.includes('service.')],
 ['new layout adds no important escalation',!css.includes('!important')],
]
let ok=0;for(const[name,pass]of checks){console.log(`${pass?'✓':'✗'} ${name}`);if(pass)ok++}

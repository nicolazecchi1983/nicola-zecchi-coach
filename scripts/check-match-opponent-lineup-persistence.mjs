...
const checks = [
  ...
  ['Service has crash-safe upload/replace lifecycle', service.includes('uploadOpponentLineup') && service.includes('beginRecovery(matchId, [{ bucket, path }])') && service.includes('previousPath = current.opponentLineup?.path') && service.includes('trackRecovery(matchId, [{ bucket: previousBucket, path: previousPath }])') && service.includes('reconcileStorageRecovery')],
  ['Service has crash-safe persisted removal lifecycle',

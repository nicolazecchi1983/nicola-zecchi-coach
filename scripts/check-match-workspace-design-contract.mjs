...
const checks=[
 ...
 ['native Match sections promote stable match identity instead of section-only title',nativeView.includes('matchTitleHtml')&&nativeView.includes('match-workspace-shell__versus')&&nativeView.includes('matchIdentityMetaHtml')&&nativeView.includes('matchMetaHtml')],
 ['native Match identity preserves home-away order and operational metadata',nativeView.includes("homeAway === 'away'")&&nativeView.includes('competitionRound')&&nativeView.includes('Giornata')&&nativeView.includes('place')],
 ...
 ['shell owns moment-scoped contextual navigation',shell.includes('matchContextNavigationHtml(activeSection')&&shell.includes('getMatchWorkflowSectionsForSection')&&shell.includes('data-match-context-navigation')],
 ...
 ['desktop contextual navigation derives PRE 4 and POST 3 columns',css.includes('.match-workspace-shell--pre-match{ --product-nav-columns:4; }')&&css.includes('.match-workspace-shell--post-match{ --product-nav-columns:3; }')&&product.includes('grid-template-columns:repeat(var(--product-nav-columns, 6),minmax(0,1fr))!important')],

  ...
  requestFrame = globalThis.requestAnimationFrame,
    ...
    if (matchEditor) {
      ...
      lineupPdfButton?.addEventListener('click', async () => {
        ...
        const resolveLeadershipName = (slot) => currentStarterEntries().find((item) => item.index === String(slot || ''))?.name || '—'
        const captain = resolveLeadershipName(form.elements.captain?.value)
        const vice = resolveLeadershipName(form.elements.vice_captain?.value)
